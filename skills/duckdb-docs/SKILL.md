...
description: >
  ...
  locally cached index. Use when the user asks about DuckDB SQL syntax,
  configuration options, functions, extensions, or performance tuning.
...
If not found, tell the user to install DuckDB (https://duckdb.org/docs/installation) and stop.
...
- **ATTACH fails / network unreachable**: inform the user that the docs index is unavailable and suggest checking their internet connection.
- **No results**: try broadening the query — drop the least specific term, or try a single-word version — then retry Step 5. If still no results, suggest visiting https://duckdb.org/docs directly.

## Step 7 — Present results

For each result chunk (ordered by score descending), format as:
...
After presenting all chunks, synthesize a concise answer to the user's original question based on the retrieved documentation.
